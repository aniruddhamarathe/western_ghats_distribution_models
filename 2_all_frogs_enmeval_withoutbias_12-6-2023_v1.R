...
##-5.max sum bin output: mss_bin.asc
##-6.mintrain bin output: mintrain_bin.asc
##-7.predictions clipped within the lowest contour: _binzones_clip.asc
##-8.predictions clipped at barrier contours: _binzones_mask.asc
##-9.folder /masked: contains outputs masked by wg
##-10.plots: contains response curves and var imp
##-11.projections: contains projections for future scenarios

#IN this script outputs upto 4.5 and 4.9 to 4.11 are created
...
####ENMevaluate####
path <- "E:/backups/all_frogs_enmeval_nobias_12-6-2023"
...
mods_current2 <- lapply(sapply(paste0(mods_current1$Binomial_species,"_bioclim_enmeval"),
                               function(x){
                                 list.files("E:/backups/all_frogs_enmeval_18-4-2023/",
                                            pattern = x,full.names = T,recursive = T)
                               }),load)
...
lapply(mods_current1$Binomial_species,
       function(spp){
         ...
         p1 <-unlist(list.files("E:/backups/all_frogs_enmeval_18-4-2023",pattern = name,full.names = T,recursive = T))
         
         t1 <- unlist(strsplit(p1,"/"))
         
         path_spp <- paste(t1[1:(length(t1)-1)],collapse = "/")
         
         load(unlist(list.files("E:/backups/all_frogs_enmeval_18-4-2023/",pattern = name,full.names = T,recursive = T)))
...
dsn = "E:/backups/all_frogs_enmeval_nobias_12-6-2023"
...
i <- which(thresh_current_avg$Binomial_species == "Raorchestes_kaikatti")
...
#Theshold predicted for Raorchestes kaikatti (0.8992) is more than the maximum value in prediction
#due to rounding. So assigning minimum trainint value upto 3 decimals only 0.8991

thresh_current_avg[thresh_current_avg$Binomial_species == "Raorchestes_kaikatti",
                   6:9] <- 0.8991
#now redo the thresholds_out for  Raorchestes kaikatti 

####save.image####
save.image("sdm/all_frogs_enmeval_without_bias/all_frogs_enmeval_without_bias_14-6-2023.RData")
